  ...
  $.getJSON('/get_origin_coords', {
    ...
    function(data) {
      ...
      for (var i = 0; i < data.length; ++i) {
        ...
        if (data[i].Year == document.getElementById("selectYear").value ||
...
info.update = function(props) {
  ...
  this._div.innerHTML = '<h4>SA1 Area</h4>' + (props ?
    '<p>' + '<b>' + 'Population: ' + '</b>' + props.DistanceMa + '</p>' :
    '<p> Hover over a SA1 area </p>');
...
document.getElementById('slider-km').onchange = function() {
  ...
  if (document.getElementById('slider-km').value == 5) {
    ...
    $(".additional_claims_desc").html(
      "additional claims expected when compared to current policy.");
  } else {
