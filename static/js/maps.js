...
function updateChoropleth_Origins(e) {
  ...
  $(".sidebar-desc").html("Persons who travel to " + lga_area.toProperCase() +
    " for medical services are from:");
...
function getTravelOriginData(key_destination_lga, selected_year) {
  ...
  $.getJSON('/get_travel_origins', {
    ...
    function(jqXHR, status, error) {
      ...
      bootbox.alert(
        "There are no records of persons travelling to that area.");
...
function updateChoroColours_Origins(layer) {
  var specific_lga_intensity = origin_choropleth_intensities[layer.feature.properties
    .vic_lga__3]
...
function getMaxValue(locations_json) {
  var values = $.map(locations_json, function(el) {
    return el;
  });
...
info.update = function(props) {
  ...
  if (CLICKED == false) {
    ...
    this._div.innerHTML = '<h4>Local Government Area</h4>' + (props ?
      '<b>' + props.vic_lga__3.toProperCase() + '</b>' + '</br>' + '<p>' +
      'Count: ' + origin_choropleth_intensities[props.vic_lga__3] + '</br>' +
      'Percentage: ' + (origin_choropleth_intensities[props.vic_lga__3] /
        total_commuters * 100).toFixed(2) + '%' + '</p>' :
      '<p> Hover over a LGA </p>');
  }
};
...
function getPieDataset(lga_counts) {
  ...
  ordered_lga_list = vectorised_lgas.sort(function(a, b) {
    return b[1] - a[1]
  })
